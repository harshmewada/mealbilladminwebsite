...
const ManageBranches = () => {
  ...
  const getBranchCode = (restaurantId) => {
    const currRestaurant = restaurants.find((res) => {
      return res._id == restaurantId;
    });
    console.log("restaurant", currRestaurant);
    return `${currRestaurant.name
      .split(" ")
      .join("")
      .slice(0, 3)
      .toUpperCase()}${(currRestaurant.branchCount + 1)
      .toString()
      .padStart(2, "0")}`;
  };
  ...
  const confirmDelete = (data) => {
    dispatch(deleteBranch(actionData)).then((res) => {
    ...
    if (open === "Add") {
      dispatch(
        createBranch({
          ...
          branchCode: getBranchCode(data.restaurantId || restaurantId),
        })
      ).then((res) => {
        if (res.payload.status === 200) {
